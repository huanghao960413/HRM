    ...
    <div class="layui-body">
        ...
        <div style="padding: 15px;">
            ...
            <c:if test="${requestScope.totalPages > 0}">
                <table>
                    <tr>
                        <td><a class="layui-btn layui-btn-sm" href="adminRecruitInformationShow?currentPage=1">首页</a></td>
                        <c:if test="${requestScope.currentPage != 1}">
                            <td><a class="layui-btn layui-btn-sm" href="adminRecruitInformationShow?currentPage=${requestScope.currentPage - 1}"><</a>
                            </td>
                        </c:if>
                        <c:forEach var="i" begin="1" end="${requestScope.totalPages}">
                            <td><a class="layui-btn layui-btn-sm" href="adminRecruitInformationShow?currentPage=${i}">${i}</a></td>
                        </c:forEach>
                        <c:if test="${requestScope.currentPage != requestScope.totalPages}">
                            <td><a class="layui-btn layui-btn-sm" href="adminRecruitInformationShow?currentPage=${requestScope.currentPage + 1}">></a>
                            </td>
                        </c:if>
                        <td><a class="layui-btn layui-btn-sm" href="adminRecruitInformationShow?currentPage=${requestScope.totalPages}">尾页</a></td>
                    </tr>
                </table>
            </c:if>
        </div>
